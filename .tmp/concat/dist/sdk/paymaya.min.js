(function(w) {
	w.PayMaya = {
		...
		addPanel: function(url, callback) {
			...
			iframeDom.width = (parseInt(paymayaBox.style.width.toString().replace('px', ''),10) - 20) + 'px';
			iframeDom.height = (parseInt(paymayaBox.style.height.toString().replace('px', ''),10) - 20) + 'px';
			...
			if (w.XMLHttpRequest) {
				...
				if (w.ActiveXObject) {
					...
					/*try {
						...
						try {
							req = new w.ActiveXObject('MSXML2.XMLHTTP.3.0');
						}
						catch (es) {
							try {
								req = new w.ActiveXObject('MSXML2.XMLHTTP');
							}
							catch (ed) {
								try {
									req = new w.ActiveXObject('Microsoft.XMLHTTP');
								}
								catch (ee) {
									req = {};
								}
